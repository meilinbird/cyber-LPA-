...
LPA Classes: 2 classes (Low Victimization / High Victimization)
"""
import matplotlib
...
print("LPA Classes: 2 classes (Low Victimization / High Victimization)")
print("=" * 80)
...
print("\n【Step 2】LPA Classification Based on t3 Cybervictimization Data (2 Classes)")
print("-" * 50)

# Check target column
if 'CYBERVICTIMIZATION_t3' not in df_raw.columns:
    cyber_t3_cols = [col for col in df_raw.columns if 'CYBER' in col.upper() and '_t3' in col]
    if len(cyber_t3_cols) > 0:
        target_col = cyber_t3_cols[0]
        print(f"Using alternative variable: {target_col}")
    else:
        print("❌ No t3 cybervictimization variables found")
        exit()
else:
    target_col = 'CYBERVICTIMIZATION_t3'

# Extract t3 cybervictimization scores
X_cluster = df_raw[[target_col]].values
print(f"Clustering data dimension: {X_cluster.shape[0]} samples × 1 time point (t3)")

# Fixed to 2 classes (remove BIC optimal number annotation)
n_classes = 2
...
# Class names (English)
class_names = ['Low Victimization Group', 'High Victimization Group']
...
# Visualization (separate plots, remove optimal number annotation)
...
plt.xlabel('t3 Cybervictimization Score')
plt.ylabel('Frequency')
plt.title('t3 Cybervictimization Score Distribution (2 Classes)')
plt.legend()
plt.tight_layout()
...
# Exclude cybervictimization variables
feature_vars = [v for v in valid_vars if 'CYBER' not in v.upper()]
...
# Select top 20 features for correlation analysis (avoid too many variables)
...
# ==================== 6. Define Models (Remove SVM, Remove "优化" label) ====================
print("\n【Step 6】Define Models")
print("-" * 50)

# Base models (remove SVM, remove "优化" from names)
base_models = {
...
if best_model is not None:
    try:
        ...
        plt.tight_layout()
        image_path = os.path.join(OUTPUT_IMAGE_DIR, '4_learning_curve_2class.png')
        plt.savefig(image_path, dpi=150, bbox_inches='tight')
        plt.close()
...
try:
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"LPA Classes: 2 (Low Victimization / High Victimization)\n\n")
...
print(f"  - LPA Classes: 2 (Low Victimization / High Victimization)")
